//将.nod.xml文件中的node的直角坐标系的x,y值利用高斯投影反算公式转换成大地坐标系中对应的经纬度，转换后输出新的.nod.xml文件
...
void GaosF_fun(double x,double y,double A[])      // 高斯投影反算公式
{      
...
int main()
{
	ifstream fin("D:\\documents\\aodv.nod.xml", ios::in);                      //打开.nod文件
